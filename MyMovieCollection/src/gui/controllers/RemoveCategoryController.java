...
public class RemoveCategoryController implements Initializable {
    ...
    public void removeCategory(ActionEvent actionEvent) throws SQLException, SqlServerException {
        String selectedIndex = categoriesBox.getSelectionModel().getSelectedItem().toString();
        CategoryDAO.removeCategory(categoryDAO.getAllCategories().filtered(e->e.getName().equals(selectedIndex)).get(0));
        categoriesBox.setItems(FXCollections.observableArrayList(categoryDAO.getCategoryName()));
        Stage stage = (Stage) closeButton.getScene().getWindow();
        stage.close();
    }
